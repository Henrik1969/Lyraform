...
static bool graph_activate(const TinyvmArtifactV2 *a,TinyvmIsaV1Context *ctx,const InstrWord *w,uint64_t instruction){
    ...
    if(ctx->graph_record_count>=ctx->graph_record_limit)return trap(ctx,TV1_TRAP_EXPLICIT,"graph activation record limit exceeded",instruction);
    if(ctx->graph_record_count==ctx->graph_record_capacity){
        size_t capacity=ctx->graph_record_capacity?ctx->graph_record_capacity*2:16;
