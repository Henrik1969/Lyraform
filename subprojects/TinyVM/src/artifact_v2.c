...
void tinyvm_artifact_v2_destroy(TinyvmArtifactV2 *a){
    ...
    free(a->code);free(a->constants);free(a->strings);free(a->storage);free(a->imports);free(a->provenance);free(a->graph_activations);tinyvm_artifact_v2_init(a);
}
...
bool tinyvm_artifact_v2_validate(const TinyvmArtifactV2 *a,char *d,size_t cap){
    ...
    if(a->graph_activation_count&&(!a->graph_activations||!ids_sorted_u64(&a->graph_activations[0].id,a->graph_activation_count,sizeof(*a->graph_activations)))){diag(d,cap,"graph activation identities are not unique and sorted");return false;}
    for(size_t i=0;i<a->graph_activation_count;++i){const TinyvmGraphActivation *v=&a->graph_activations[i];if(!text64(v->kind)||!text64(v->node_id)||!text64(v->wire_id)||!text64(v->input_port)||!text64(v->output_port)){diag(d,cap,"invalid graph activation metadata");return false;}}
    diag(d,cap,"valid");return true;
}

static size_t sections_for(const TinyvmArtifactV2 *a,Section s[7],char *d,size_t cap){
    ...
    if(a->graph_activation_count){if(!mul_size(a->graph_activation_count,368,&bytes)){diag(d,cap,"graph activation section overflow");return 0;}s[n++]=(Section){SEC_GRAPH_ACTIVATIONS,0,bytes,a->graph_activation_count};}
...
static void encode_sections(const TinyvmArtifactV2 *a,uint8_t *b,const Section *s,size_t n){
    for(size_t k=0;k<n;++k){uint8_t *p=b+s[k].offset;
        ...
        else if(s[k].type==SEC_GRAPH_ACTIVATIONS)for(size_t i=0;i<a->graph_activation_count;++i){uint8_t *r=p+i*368;const TinyvmGraphActivation *v=&a->graph_activations[i];p64(r,v->id);p64(r+8,v->activation_id);p64(r+16,v->input_activation_id);p64(r+24,v->input_signal_id);p64(r+32,v->output_signal_id);p64(r+40,v->delivery_id);memcpy(r+48,v->kind,64);memcpy(r+112,v->node_id,64);memcpy(r+176,v->wire_id,64);memcpy(r+240,v->input_port,64);memcpy(r+304,v->output_port,64);}
    }
}
...
static bool parse_directory(const uint8_t *b,size_t size,Section s[7],size_t *count,char *d,size_t cap){
    uint32_t n=g32(b+424);if(n<2||n>7||g64(b+416)!=V2_HEADER||g32(b+428)!=32||size<V2_HEADER+(size_t)n*32){diag(d,cap,"invalid section directory header");return false;}
    uint64_t expected=V2_HEADER+(uint64_t)n*32;uint32_t previous=0;
    for(uint32_t i=0;i<n;++i){const uint8_t *e=b+V2_HEADER+i*32;s[i]=(Section){g32(e),g64(e+8),g64(e+16),g64(e+24)};expected=(expected+7)&~UINT64_C(7);if(g32(e+4)||s[i].type<=previous||s[i].type>7||s[i].offset!=expected||s[i].offset>size||s[i].size>size-s[i].offset){diag(d,cap,"invalid section directory entry");return false;}if(!zeros(b+(size_t)(i? s[i-1].offset+s[i-1].size : V2_HEADER+n*32),(size_t)(s[i].offset-(i? s[i-1].offset+s[i-1].size : V2_HEADER+n*32)))){diag(d,cap,"section alignment padding is nonzero");return false;}expected=s[i].offset+s[i].size;previous=s[i].type;}
    if(expected!=size||s[0].type!=SEC_CODE||(s[n-1].type!=SEC_PROVENANCE&&s[n-1].type!=SEC_GRAPH_ACTIVATIONS)){diag(d,cap,"required sections or exact coverage missing");return false;}*count=n;return true;
}
...
bool tinyvm_artifact_v2_read(const char *path,TinyvmArtifactV2 *a,char *d,size_t cap){
    ...
    x=find_section(s,n,SEC_GRAPH_ACTIVATIONS);if(x){if(x->count>SIZE_MAX/368||x->size!=x->count*368){free(b);tinyvm_artifact_v2_destroy(a);diag(d,cap,"graph activation section malformed");return false;}a->graph_activation_count=(size_t)x->count;a->graph_activations=calloc(a->graph_activation_count,sizeof(*a->graph_activations));if(!a->graph_activations){free(b);tinyvm_artifact_v2_destroy(a);diag(d,cap,"allocation failed");return false;}for(size_t i=0;i<a->graph_activation_count;++i){const uint8_t *p=b+x->offset+i*368;TinyvmGraphActivation *v=&a->graph_activations[i];v->id=g64(p);v->activation_id=g64(p+8);v->input_activation_id=g64(p+16);v->input_signal_id=g64(p+24);v->output_signal_id=g64(p+32);v->delivery_id=g64(p+40);memcpy(v->kind,p+48,64);memcpy(v->node_id,p+112,64);memcpy(v->wire_id,p+176,64);memcpy(v->input_port,p+240,64);memcpy(v->output_port,p+304,64);}}
