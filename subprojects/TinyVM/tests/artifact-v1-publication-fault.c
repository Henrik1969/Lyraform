...
static void require_protocol_temporary(const char *directory) {
    char path[1024];
    const int length = snprintf(path, sizeof path, "%s/publication-v1.tvm.tmp.lyraform-v1", directory);
...
int main(int argc, char **argv) {
    ...
    if (child == 0) {
        ...
        (void)tinyvm_artifact_write_result(path, &artifact, diagnostic, sizeof diagnostic);
        _exit(87);
    }
    int child_status = 0;
