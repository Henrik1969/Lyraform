...
| `std/abi/file_io.flow` | C file descriptor I/O | `open`, `read`, `write`, `sendfile`, and `close` resolve through exact grants; the `open`/`close` pair now executes with LLVM and TinyVM descriptor tracking, while `flowcat` uses generic typed-plan loops, branches, mutation, and calls | bounded LLVM/TinyVM resource slice; Linux-specific `sendfile` example |
