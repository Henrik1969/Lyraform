...
| `std/abi/file_io.flow` | C file descriptor I/O | `open`, bounded `read`/`write`, `sendfile`, and `close` resolve through exact grants; LLVM and TinyVM execute tracked descriptors with eight-byte storage, including EOF and `/dev/full` error returns, while `flowcat` uses generic typed-plan loops, branches, mutation, and calls | bounded LLVM/TinyVM resource/I/O slice; Linux-specific `sendfile` example |
