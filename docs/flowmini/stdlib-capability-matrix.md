...
| `std/abi/text.flow` | Bounded owned Text concatenation and recovery status | `flow_text_concat(Text,Text)->Text` is provider-owned, capped at 4096 bytes, and exercised through exact `libflowtext.so` authorization; LLVM and TinyVM cover success, exhaustion, and explicit Flow status branching | bounded native/TinyVM slice; atomic Outcome pending |
