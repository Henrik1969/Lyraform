...
- The exact aggregate manifest identity is carried through Flowbind,
  ...
  or evidence.
- Aggregate values are immutable graph payloads for this phase. Bounded finite
  stream v2 delivery is admitted for verified one-64-bit aggregate items; aggregate
  mutation, joins, and parallel aliasing remain out of scope.
...
general ABI lowering path is separately completed. Aggregate stream delivery is
limited to the bounded finite-stream-v2 one-64-bit carrier contract; joins and
parallel/reentrant aggregate delivery remain out of scope. Persistent aggregate
state remains admitted through the bounded schedule-v3 one-64-bit carrier contract.
