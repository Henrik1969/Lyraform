...
| source graph / finite scalar or verified aggregate stream | one exactly authorized read-only count `()->c_size_t` and item `(c_size_t)->c_int` or verified packed aggregate provider, bounded item loop, direct root-to-receiver fan-out and linear type-continuous receiver-pipeline parity with LLVM; aggregate pipeline coverage is exercised by `tinyvm_aggregate_stream_pipeline_parity` |
