...
| Hostile plan/report input exhausts a planning boundary | Current Flowparallel planning/report readers and active artifact tools cap stdin and file input at 16 MiB before parsing; oversized input emits structured no-artifact failure | `flowparallel_exception_containment` oversized-ingress cases; artifact boundary suites | implemented for covered active ingress points | Flowanalyst and compiler source ingress still require independently tested limits |
