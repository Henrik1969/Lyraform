...
| File/terminal/native-library resources leak or close twice | Path-aware cleanup validation and explicit ownership metadata; CUDA device/cuBLAS handles use repeatable status-returning cleanup | `file_resource_boundary`, `file_io_boundary`, `sel_tui_pipeline`, `flowparallel_cuda_resource_cleanup` | provisional | CUDA graph/matrix dynamic-library `dlclose` failures are not yet represented; full cancellation cleanup is not yet admitted |
