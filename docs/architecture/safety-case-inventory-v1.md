...
|---|---|---|---|---|
| Malformed or truncated stage artifact is consumed | Typed version/field/identity validation at each consumer, including Flowparallel CPU and graph-reference consumers; v2 loader allocation exhaustion refuses before publishing ownership | `flowcontracts_json`, `flowvalidate_artifacts`, `tinyvm_artifact_v2_hostile`, `tinyvm_artifact_v2_allocation_fault`, `flowparallel_cpu_provider`, `flowparallel_graph_reference` | implemented | New artifact fields require the same hostile coverage |
