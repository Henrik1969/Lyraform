...
| Hostile plan/report input exhausts a planning boundary | Flowparallel plan, CPU/CUDA provider, graph CPU/CUDA report readers, and runtime/graph planners cap stdin and file input at 16 MiB before JSON parsing; oversized input emits structured no-artifact failure | `flowparallel_exception_containment` oversized-ingress cases | implemented for current Flowparallel ingress points | Other native readers require their own contract-sized limits |
