...
| Native package metadata causes unbounded observation allocation | Read-only package providers cap each dpkg status paragraph at 1 MiB and each APT metadata file at 4 MiB; oversized records/files are diagnosed and skipped | `frankencore_packages_probe` oversized-record and oversized-metadata cases | implemented for per-record/file bounds | Aggregate inventory cardinality needs a separate bounded contract |
