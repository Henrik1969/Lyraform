...
| Graph diagnostics exhaust stderr or log storage | Graph runtime diagnostics share a 16 MiB aggregate output budget and emit one structured truncation marker after exhaustion | `flowgraph_runtime_bounds` diagnostic-output case | implemented for the current graph runtime | External log retention remains an operator concern |
...
| Partial graph worker launch leaves joinable threads | Graph runtime uses joining thread ownership so vector-growth or later launch failure cleans up already-started workers | `flowgraph_runtime_bounds` launch-path evidence | implemented for the current graph runtime | Allocation-failure injection remains a separate evidence gap |
