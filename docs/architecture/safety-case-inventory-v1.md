...
| File/terminal/native-library resources leak or close twice | Path-aware cleanup validation and explicit ownership metadata; CUDA device/cuBLAS handles and all current CUDA dynamic-library paths use repeatable status-returning cleanup | `file_resource_boundary`, `file_io_boundary`, `sel_tui_pipeline`, `flowparallel_cuda_resource_cleanup`, `flowparallel_dynamic_library_cleanup`, `flowparallel_cuda_execute_allocation_fault`, CUDA provider/graph/benchmark focused gates | provisional | Broader native cleanup fault injection and full cancellation cleanup are not yet admitted |
