...
| Build-host hardware is mistaken for deployment capability | Runtime snapshot separated from compile-time policy; checked discovery returns no admitted snapshot on allocation, standard, or unknown failure, and CUDA probe cleanup failure downgrades evidence to unknown | runtime capability and provider-planner tests, `frankencore_runtime_probe_fault` | implemented | Runtime refresh, hardware-independent CUDA close-fault injection, and broader platform matrix remain open |
