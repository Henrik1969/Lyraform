...
| Provider handle leaks across an exceptional binding path | Flowbind owns every admitted `dlopen` handle with an explicit RAII deleter, including failure and allocation-unwind paths; TinyVM closes a newly opened descriptor when ownership-table growth exhausts memory | `flowbind_provider` normal, missing-symbol, and hostile ABI cases; `tinyvm_runtime_provider_allocation_fault` | implemented for current binding inspection | Native provider execution remains separately bounded and provisional |
