...
| Native package metadata causes unbounded observation allocation | Read-only package providers cap each dpkg status paragraph at 1 MiB and report oversized records as diagnosed, skipped observations | `frankencore_packages_probe` oversized-record case | implemented for dpkg record bounds | APT metadata and aggregate inventory cardinality need separate bounded contracts |
