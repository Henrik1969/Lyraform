...
| Unsupported platform or skipped capability is presented as complete | Host-specific Linux x86-64 baseline and machine-readable Flowkernel result; default reports skips explicitly and `--require-complete` refuses any skipped probe | `2026-09-14-platform-assurance-linux-x86-64.md`; `flowkernel_all`; `flowkernel_require_complete` | provisional | This host reports loopback `EPERM`; cross-platform assurance and complete permission profile remain open |
