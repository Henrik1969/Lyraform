...
| Bootstrap implementation exception crosses a public semantic boundary | Stage 0 C++ `DiagnosticError` paths must be translated to structured stage/runtime results before artifact or language semantics are exposed | Current audit found internal throws/catch/rethrow in `flowmini_runtime.cpp`; public CLI catches at the outer boundary | provisional | Exception containment and no-exception public ABI need a dedicated Gate 2 test; uncaught allocation/library exceptions remain an audit item |
