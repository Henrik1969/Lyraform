...
| Bootstrap implementation exception crosses a public semantic boundary | Stage 0 C++ exceptions must be translated to structured stage/runtime results before artifact or language semantics are exposed | `structured_cli_diagnostics` proves CLI translation for known diagnostics; current audit found internal throws/catch/rethrow in `flowmini_runtime.cpp` | provisional | Provider/runtime/API containment, stable stage codes, and injected allocation/library exception coverage remain open |
