...
| Language moniker resolution crosses its public boundary | Moniker input is capped at 4096 bytes and resolver exceptions become an unresolved structured diagnostic rather than escaping the Frankencore language API | `frankencore_language_probe` oversized-input case | implemented for the current resolver boundary | Allocation-fault injection and other Frankencore/provider APIs remain open |
