...
| File/terminal/native-library resources leak or close twice | Path-aware cleanup validation and explicit ownership metadata; CUDA device/cuBLAS handles and the CUDA execution provider’s dynamic libraries use repeatable status-returning cleanup | `file_resource_boundary`, `file_io_boundary`, `sel_tui_pipeline`, `flowparallel_cuda_resource_cleanup`, `flowparallel_dynamic_library_cleanup`, `flowparallel_cuda_execute_allocation_fault` | provisional | CUDA graph/matrix benchmark dynamic-library `dlclose` failures are not yet represented; full cancellation cleanup is not yet admitted |
...
| Build-host hardware is mistaken for deployment capability | Runtime snapshot separated from compile-time policy; checked discovery returns no admitted snapshot on allocation, standard, or unknown failure | runtime capability and provider-planner tests, `frankencore_runtime_probe_fault` | implemented | Runtime refresh and broader platform matrix remain open |
