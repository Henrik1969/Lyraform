...
| Text allocation failure is represented as success | Tagged `Outcome<Text,TextFailure>` boundary; bounded provider allocation failure returns `FLOW_TEXT_EXHAUSTED` without publishing a value, and TinyVM provider-owned outcome allocation refuses with an uninitialized result | `text_outcome_boundary`, `tinyvm_runtime_text_parity`, `flowtext_runtime_allocation_failure`, `tinyvm_runtime_provider_allocation_fault` | implemented for the bounded Text/provider boundary | Wider owned-text semantics and other native allocators remain scoped |
