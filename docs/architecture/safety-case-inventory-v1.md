...
| Native package metadata causes unbounded observation allocation | Read-only package providers cap each dpkg status paragraph at 1 MiB, each APT metadata file at 4 MiB, and each result collection at 100,000 entries; oversized records/files, cardinality overflow, malformed provider rows, and provider exit failure are diagnosed | `frankencore_packages_probe` oversized-record, oversized-metadata, cardinality, and provider-failure cases | implemented for current package-provider collections | Broader provider schemas may require independently sized limits |
