...
| Public contract collections exhaust admission consumers | Language-map entries, aliases, chain-policy prerequisites/targets, and facade argument/diagnostic/provenance collections reject more than 100000 items; text entries remain bounded | `frankencore_conformance` hostile policy and facade cases | implemented for the current contract boundary | Serialized consumers retain schema-specific limits |
