...
set -e
test "$status" -eq 1
jq -e '.status == "failed" and .code == "FLOWPARALLEL_MATRIX_BENCHMARK_INPUT_INVALID" and .stage == "input" and .disposition == "no_artifact" and (.message | length > 0)' "$diagnostic_err" >/dev/null
set +e
"$benchmark" --size 64junk --diagnostics json >/dev/null 2>"$diagnostic_err"
status=$?
set -e
test "$status" -eq 1
jq -e '.code == "FLOWPARALLEL_MATRIX_BENCHMARK_INPUT_INVALID" and .stage == "input" and (.message | contains("complete integer"))' "$diagnostic_err" >/dev/null
