...
inline void write_json_string(std::FILE* output, std::string_view value) noexcept {
    ...
    const auto truncated = [&] { if (emitted <= max_diagnostic_bytes - 3) put("...", 3); };
    for (const unsigned char character : value) {
        ...
        default:
            if (character < 0x20) {
                ...
                control_escape[0] = '\\'; control_escape[1] = 'u'; control_escape[2] = '0'; control_escape[3] = '0';
                control_escape[4] = hex[(character >> 4) & 0xf]; control_escape[5] = hex[character & 0xf];
                escaped = control_escape; escaped_length = 6;
            } else {
                if (emitted >= max_diagnostic_bytes - 3) { truncated(); return; }
                (void)std::fputc(character, output); ++emitted; continue;
            }
        }
        if (emitted > max_diagnostic_bytes - 3 || escaped_length > max_diagnostic_bytes - 3 - emitted) {
            truncated(); return;
        }
