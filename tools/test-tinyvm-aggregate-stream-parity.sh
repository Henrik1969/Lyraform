...
program tinyvm_aggregate_stream
...
fn consume_long(value : LongValue): c_int {
    ...
    if result == 42 {
        libc.puts("aggregate stream forty-two") -> result
    } else {
        libc.puts("aggregate stream forty-three") -> result
    }
    return result
}
main { return 0 }
...
printf 'aggregate stream forty-two\naggregate stream forty-three\n' > "$tmpdir/expected.stdout"
cmp "$tmpdir/expected.stdout" "$tmpdir/llvm.stdout"
jq -e '.graph_schedule.version == 2 and .graph_schedule.stream_contract == "finite_aggregate_stream_v1" and .graph_schedule.streams[0].item_output_type == "LongValue"' "$tmpdir/execution.json" >/dev/null
...
echo 'TinyVM aggregate stream parity: PASS'
